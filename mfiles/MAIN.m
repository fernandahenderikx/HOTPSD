...
%% get averages
%ok, Biovolume partitioned by both taxonomic identity and cell size.
...
for i = 1:length(f)
    ...
    abund_summer(i,:) = nanmean(trapz(psdCSA_aloha(summer,:),2));
    abund_summer_se(i,:) = nanstd(trapz(psdCSA_aloha(summer,:),2))./sqrt(length(summer));
    abund_winter(i,:) = nanmean(trapz(psdCSA_aloha(winter,:),2));
    abund_winter_se(i,:) = nanstd(trapz(psdCSA_aloha(winter,:),2))./sqrt(length(winter));
    abund_spring(i,:) = nanmean(trapz(psdCSA_aloha(spring,:),2));
    abund_spring_se(i,:) = nanstd(trapz(psdCSA_aloha(spring,:),2))./sqrt(length(spring));    
    abund_fall(i,:) = nanmean(trapz(psdCSA_aloha(fall,:),2));
    abund_fall_se(i,:) = nanstd(trapz(psdCSA_aloha(fall,:),2))./sqrt(length(fall));
end
...
%% Fig 4, numerically, how many particles per liter per class
...
xlim([0.5 1.5]); ylim([6*10^4 14*10^4 ])
...
%% Fig 6  (example summer sub categories); D
figure(444)
...
%% Figure S4 (same as Fig 6 but all seasons)
figure(4441)
set(gcf,'Position',[68    50   926   946])
...
% regressions against SD50 and D90
runTaxonMomentsStats_MAIN
% structural shape sensitivity test
FORPAPER_mean_shape_contribution_tests

%% make csv for per phylum psds (volume) per cruise
...
%% make csv for per phylum psds (abundances) per cruise
clearvars -except max_good_diam*  good_cruises figsdir nsamples_percruise
f = dir([datadir filesep 'CNN_TS4_2025-02-26_merged_Group1'])
f([1:2 13 15])=[];
%f([1:2])=[];
